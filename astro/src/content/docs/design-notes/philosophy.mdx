...
John Company is a complex game and _{frontmatter._["game.title"]}_ adds even more rules into the system. Therefore, this expansion is only for those who have played multiple games of the other scenarios.
...
- The original scenarios can be switched between without much impact.
    ...
    The hope is that there is little need to shuffle in (or remove) scenario-specific cards from the existing decks, other than locating a specific card or two during setup.
