...
:::danger[Firm Incentives]
What is the incentive for the firms to vote for this? In the base scenario, an anti-company player can vote for deregulation, and gets the benefit of being able to start or invest in a firm. In this scenario, the firms would be adding the Company as competition to themselves by voting for this, which means they would almost certainly vote against it.

The simple answer might be there is no incentive. Instead, the Company has to overcome the Firms political power by having an amount of supporting votes to override them. An example would be to base the starting votes on the [amount of tea](#managing-the-tea) that the Company has in its warehouses. When the Company has a large reserve of tea, this would allow the Company to successfully pass it without the firms' support.

Similar to deregulation, the vote would only be allowed during certain conditions, such as when the Company Standing or Company Debt tracks are on a lined space.
:::
...
There are currently two candidate names for this office:
...
:::tip[Idea]{icon="approve-check-circle"}
